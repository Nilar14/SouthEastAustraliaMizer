...
multispeciesParams <- function(object, interaction,
    ...
    ## AA Warnings for temperature scalar parameters 
...
#    }
    ...
    ### CN this is only because gear_names changes and cannot be a column of object  
    ...
    if(fleetDynamics==TRUE){
      ...
      for (fleet_count in fleet) 
        ...
        for (sp_count in as.character(unique(selectivity_params[selectivity_params$subfleet==fleet_count,"species"]))){
          ...
          # find the selectivity function for that specific fleet/gear and species (e.g. trawl)
      ...
      # check or re-arrange species and fleet order in selectivity - this should not be necessary...
      ...
      # no need to fill these matrices up (as done below) but just re-arrange the order of species and fleet to match that in object and selectivity   
